...
public class Concord
{
    ...
    public void printSorted(PrintStream out) {
        ArrayList<Map.Entry<String, Integer>> sortResult= new ArrayList<Map.Entry<String, Integer>>();
        for (Map.Entry<String, Integer> curr : concord.entrySet()) {
            sortResult.add(curr);
        }
        Collections.sort(sortResult,new MapEntryComparator());
        for(int i = 0 ; i < sortResult.size();i++) {
            out.println(sortResult.get(i).getKey() + " " + sortResult.get(i).getValue());
        }
    }
